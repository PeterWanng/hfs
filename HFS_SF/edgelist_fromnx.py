...
def parse_edgelist(lines, comments='#', delimiter=None,
                   create_using=None, nodetype=None, data=True, justkeepcore=False):
...
#             line = line[:p]
        ...
        if len(s)<2:
            continue
        "=========================ignore the not core edges:just forward with no @,//@"
#         linestr = line
#         linestr = linestr.split(',')
        if justkeepcore and str(line[5])=='0' and str(line[6])=='0':
            continue
        u=s.pop(0)
        v=s.pop(0)
        d=s
...
def read_edgelist(path, comments="#", delimiter=None, create_using=None, 
                  nodetype=None, data=True, edgetype=None, encoding='utf-8',justkeepcore=False):
