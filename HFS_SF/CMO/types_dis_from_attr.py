...
def getLabels(att,workfolder_fig,k=6):
    ...
    labels,res = [],[] 
#             print len(xy),xy
    labels,kmcenter,kmfit = netky.kmeans(xy,k=k,runtimes=100)
    #             labels.sort()
...
def start(labels,k,outFolder):
    workfolder_fig = outFolder#gt.createFolder("N:\\HFS\\WeiboData\\CMO\\Modefigs\\fig\\kmeans-"+str(k)+'\\')
    personcnt = 200
    experimenTimes = 100
...
#     labels = getLabels(att,workfolder_fig,k)
#     labels = np.genfromtxt('N:\\HFS\\WeiboData\\CMO\\Modefigs\\'+str(personcnt)+'_'+str(experimenTimes)+'.attlabel',dtype='int')
    labels = np.genfromtxt(r'G:\HFS\WeiboData\HFSWeibo_Sim\Output\real.attlabel',dtype='int')
    start(labels,k)
