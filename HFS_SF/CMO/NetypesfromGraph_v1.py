...
#         print assor
    ...
    #             result.append([filen,lendegabove/deglen,np.average(deg_abovepart)/deglen,np.std(deg_abovepart)/deglen,nodedis])#len(deg[avgdegindex_above:]),
        ...
        result3 = self.net_tree_star_line(gg)
...
#             filen = str(filen).replace('.png','')
            ...
            if os.path.isfile(gmlfolder+filen) and os.path.splitext(filen)[-1]=='.gml' and os.stat(gmlfolder+filen).st_size<100000000000:
                resultone = [filen]
                filesize = os.path.getsize(gmlfolder+filen)
                print filen,filesize
                g = ig.Graph.Read_GML(gmlfolder+filen)
                rs = self.featuresofgraph(g)
                resultone.extend(sp.nan_to_num(self.featuresofgraph(g)))#(self.net_tree_star_line(g)))#
                result.append(resultone)
                print rs
                rss = self.net_tree_star_line(g)
                print rss
        return result        
...
if __name__=='__main__':    
    ...
    gmlfolder = 'G:\\HFS\\WeiboData\\HFSWeiboGMLNew\\shape\\'#test\\
    pngfolder = 'G:\\HFS\\WeiboData\\HFSWeiboGMLNew\\shape\\tesTypes\\'
