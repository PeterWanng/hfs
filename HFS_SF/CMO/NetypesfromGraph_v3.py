...
#                 resultone.extend(sp.nan_to_num(self.featuresofgraph(g)))
    ...
    def define_metrics_adj_one(self,adjfile,ispanningtree=False):
        resultone = []
        if os.path.isfile(adjfile) and os.path.splitext(adjfile)[-1]=='.adj':
            resultone = [adjfile]
#             print adjfile
            g = ig.Graph.Read_Adjacency(adjfile)
            resultone.extend(sp.nan_to_num(self.featuresofgraph(g,spanningtree=ispanningtree)))
#                 resultone.extend(sp.nan_to_num(self.net_tree_star_line(g)))
        return resultone
                
    def define_metrics_adj(self,adjfolder,ispanningtree=False):
        result = []            
        for filen in os.listdir(adjfolder):
            resultone = self.define_metrics_adj_one(adjfolder+filen,ispanningtree=False)
            result.append(resultone)
        return result      
    '----------------------------------------------------------------------------------------------------------------------------------'
...
#         print resline
        np.savetxt(fname=attfilepath, X=xy, fmt='%.18e', delimiter=' ', newline='\n', header='', footer='', comments='# ')#
...
if __name__=='__main__': 
    ...
    personcnt = 100
    experimenTimes = 100
    workfolder_fig = "D:\\HFS\\WeiboData\\CMO\\Modefigs\\"
    attfilepath = workfolder_fig+str(personcnt)+'_'+str(experimenTimes)+'.att'
    netky = NetypesfromGraph() 
    for mode in range(2,8):
        subx = []
        workfolder = "D:\\HFS\\WeiboData\\CMO\\Mode"+str(mode)+"\\graphs\\"#"G:\\HFS\\WeiboData\\CMO\\test\\"#
                ...
                for modeal in [1,2,4]:#,2,4]:#3,,5
                    for modemen in [1,2,4]:#,2,4]:#3,,5
                        ...
                        #     x = netky.define_metrics_gml(gmlfolder,pngfolder)
                            ...
                            resultone = netky.define_metrics_adj_one(filepath,ispanningtree=ispt)
